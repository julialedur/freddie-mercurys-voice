...
function ready(datapoints) {
  ...
  holder.selectAll('.graph').data(nested).enter().append('div').style('display', 'inline-block').each(function (d) {
    ...
    svg.append('circle').attr('r', 3).attr('fill', 'white').attr('cx', 0).attr('cy', 0); // svg
    ...
    function animateSVG(button) {
      ...
      if (animateAllowed) {
        if (svgIsAnimated) {
          ...
          // update currentSVG
          currentSVG = svgToAnimate; // animate currentSVG
        } else {// find proper svg to animate
            // update currentSVG
            //animate currentSVG
          }
      }
...
//# sourceMappingURL=/freddie-mercurys-voice/chart-3b.53a0da37.map