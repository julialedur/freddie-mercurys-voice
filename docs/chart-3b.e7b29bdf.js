...
},{"./dist/package":"pT13","d3-array":"K0bd","d3-axis":"mp0m","d3-brush":"tkh5","d3-chord":"Iy8J","d3-collection":"S3hn","d3-color":"Peej","d3-contour":"5SiB","d3-dispatch":"D3zY","d3-drag":"kkdU","d3-dsv":"EC2w","d3-ease":"pJ11","d3-fetch":"grWT","d3-force":"oYRE","d3-format":"VuZR","d3-geo":"/Ah6","d3-hierarchy":"Kps6","d3-interpolate":"k9aH","d3-path":"9OTy","d3-polygon":"H15P","d3-quadtree":"lUbg","d3-random":"Gz2j","d3-scale":"zL2z","d3-scale-chromatic":"1ado","d3-selection":"//ys","d3-shape":"9maw","d3-time":"0hQY","d3-time-format":"UYpZ","d3-timer":"rdzS","d3-transition":"UqVV","d3-voronoi":"rLIC","d3-zoom":"MHdZ"}],"uXP7":[function(require,module,exports) {
"use strict";

module.exports = "/freddie-mercurys-voice/top-10-songs-notes.c353bd9c.csv";
},{}],"ZVb1":[function(require,module,exports) {
"use strict";
...
var radius = 70;
var radiusScale = d3.scaleLinear().domain([0, 12]).range([0, radius]);
var angleScale = d3.scaleBand().range([0, Math.PI * 2]);
var colorScale = d3.scaleLinear().domain([0, 12]).range(['#FFC75C', '#FF446E']);
var arc = d3.arc().innerRadius(function (d) {
  return radiusScale(0);
}).outerRadius(function (d) {
  return radiusScale(d.note_number);
}).startAngle(function (d) {
  return angleScale(d.second);
}).endAngle(function (d) {
  return angleScale(d.second) + angleScale.bandwidth();
});
var xPositionScale = d3.scaleBand().range([0, width]);
var yPositionScale = d3.scaleBand().range([height, 0]);
d3.csv(require('./data/top-10-songs-notes.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  var seconds = nested.forEach(function (data) {
    data.seconds = data.values.map(function (d) {
      return d.second;
    });
  }); // dictionary to connectButton to SVG element

  var buttonToSVG = {};
  holder.selectAll('.graph').data(nested).enter().append('div').style('display', 'inline-block').each(function (d) {
    var svg = d3.select(this).append('svg').style('display', 'block').style('margin', '0 auto').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', "translate(" + width / 2 + "," + height / 2 + ")");
    d3.select(this).append('button').style('display', 'block').style('margin', '0 auto').attr('class', 'button-song btn btn-outline-light btn-sm').attr('id', d.key.replace(/ /g, '-').toLowerCase()).text(d.key);
    var songButtonID = d.key.replace(/ /g, '-').toLowerCase();
    var songButton = document.getElementById(songButtonID);
    buttonToSVG[songButton] = svg;
    console.log('build');
    console.log(d);
    angleScale.domain(d.seconds);
    svg.selectAll('.temp-bar').data(d.values).enter().append('path').attr('d', function (d) {
      return arc(d);
    ...
    var circID = 'circle' + songButtonID;
    svg.append('circle').attr('r', 3).attr('fill', 'white').attr('cx', 0).attr('cy', 0).attr('id', circID); // svg
  ...
  function clickButton() {
    var buttons = document.getElementsByClassName('button-song btn btn-outline-light btn-sm');
    HTMLCollection.prototype.forEach = Array.prototype.forEach;
    buttons.forEach(function (song_button) {
      console.log('yo!'); // console.log(song_button.id)
      ...
      if (animateAllowed) {
        if (svgIsAnimated) {
          if (circID == currentSVGid) {
            ...
            oldcircle.style.transform = 'scale(1)'; // animate new 
          ...
          // update currentSVG
          //animate currentSVG
          console.log(circID); //increase size of circle
    ...
    function songButtonPressed(song_button) {
      var song = loadSongByButton(song_button);
      playSong(song);
    ...
    function loadSongByButton(song_button) {
      var songID = 'song_' + song_button.id;
      console.log(songID);
  ...
  clickButton(); //setInterval(updateScale(), 1000)
...
},{"d3":"UzF0","./data/top-10-songs-notes.csv":"uXP7"}]},{},["ZVb1"], null)
//# sourceMappingURL=/freddie-mercurys-voice/chart-3b.ade85083.map