...
function ready(datapoints) {
  ...
  holder.selectAll('.graph').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', "translate(" + width / 2 + "," + height / 2 + ")").each(function (d) {
    console.log('build');
    console.log(d);
    ...
    svg.append('circle').attr('r', 3).attr('fill', 'white').attr('cx', 0).attr('cy', 0);
    svg // .selectAll('#button-song')
    .append('text').text(function (d) {
      return d.key;
    }).attr('x', xPositionScale(d.songs)).attr('y', height / 3).attr('font-size', 15).attr('fill', 'white').attr('text-anchor', 'middle').attr('alignment-baseline', 'center');
  });
}
},{"d3":"UzF0","./data/top-10-songs-notes.csv":"uXP7"}]},{},["ZVb1"], null)
//# sourceMappingURL=/freddie-mercurys-voice/chart-3b.2c9db602.map