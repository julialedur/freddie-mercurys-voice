...
},{}],"ZDPR":[function(require,module,exports) {
"use strict";

module.exports = "/freddie-mercurys-voice/crown.4293e274.csv";
},{}],"CBw5":[function(require,module,exports) {
"use strict";

module.exports = "/freddie-mercurys-voice/circle.5d9c206c.csv";
...
Promise.all([d3.csv(require('./data/all_songs.csv')), d3.csv(require('./data/points.csv')), d3.csv(require('./data/face.csv')), d3.csv(require('./data/crown.csv')), d3.csv(require('./data/circle.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  var _ref2 = _slicedToArray(_ref, 5),
      ...
      crownCoordinates = _ref2[3],
      circleCoordinates = _ref2[4];
  ...
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 4).attr('fill', '#FA0DE5').attr('cx', function (d) {
    return d.faceX;
  }).attr('cy', function (d) {
    return d.faceY;
  });
  d3.select('#freddie').on('stepin', function () {
    svg.selectAll('circle').transition().duration(500).ease(d3.easeBack).attr('cx', function (d) {
      return d.faceX;
    }).attr('cy', function (d) {
      return d.faceY;
    });
  });
  d3.select('#crown').on('stepin', function () {
    svg.selectAll('circle').transition().duration(500).ease(d3.easeBack).attr('cx', function (d) {
      return d.crownX;
    }).attr('cy', function (d) {
      return d.crownY;
  ...
  d3.select('#circle').on('stepin', function () {
    svg.selectAll('circle').transition().duration(1000).ease(d3.easeBack).attr('cx', function (d) {
      return d.circleX;
    }).attr('cy', function (d) {
      return d.circleY;
    });
  });
  d3.select('#color').on('stepin', function () {
...
},{"d3":"UzF0","./data/all_songs.csv":"4qT6","./data/points.csv":"9VpW","./data/face.csv":"HpJq","./data/crown.csv":"ZDPR","./data/circle.csv":"CBw5"}]},{},["Uc75"], null)
//# sourceMappingURL=/freddie-mercurys-voice/chart-1.99a896b5.map